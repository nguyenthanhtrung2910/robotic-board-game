...
from Game import Board, Clock, Robot
...
class Game:
    ...
    color_map = {'b':(0,0,255), 'r':(255,0,0), 'y':(255,255,0), 'g':(0,255,0), 'o':(255,165,0)}

    def __init__(self, board: Board, required_mail: int, number_robots_with_same_color: int, robot_colors: list[str]) -> None:

        self.screen = pygame.display.set_mode((CELL_SIZE[0]*28, CELL_SIZE[1]*11)) 
        pygame.display.set_caption('Robotics Board Game') 
        self.game_clock = Clock()
        self.robot_sprites = pygame.sprite.Group()
        self.mail_sprites = pygame.sprite.Group()
        ...
        robot_cells_init = random.sample(self.board.white_cells, k = self.number_robots)
        self.robots = {robot_color:[Robot(robot_cells_init[number_robots_with_same_color*j+i], i+1, robot_color, self.mail_sprites, self.game_clock) 
                       for i in range(number_robots_with_same_color)] for j,robot_color in enumerate(robot_colors) }
        ...
        #draw axes
        for i in range(self.board.size):     
            self.background.blit(self.images_for_cell_coordinate[i], ((i+1)*CELL_SIZE[0]+(CELL_SIZE[0]-self.images_for_cell_coordinate[i].get_width())/2, 
                                                                    (CELL_SIZE[1]-self.images_for_cell_coordinate[i].get_height())/2))
            self.background.blit(self.images_for_cell_coordinate[i], ((CELL_SIZE[0]-self.images_for_cell_coordinate[i].get_width())/2, 
                                                                    (i+1)*CELL_SIZE[1]+(CELL_SIZE[1]-self.images_for_cell_coordinate[i].get_height())/2))
        #draw baterry bar
        for j in range(self.number_robots):
            for i in range(MAXIMUM_ROBOT_BATTERY+1):
                rect = pygame.Rect(10.5*CELL_SIZE[0]+i*CELL_BATTERY_SIZE[0], 
                                  (CELL_SIZE[1]*11 - CELL_BATTERY_SIZE[1]*self.number_robots)/2 +j*CELL_BATTERY_SIZE[1], 
                                   CELL_BATTERY_SIZE[0], 
                                   CELL_BATTERY_SIZE[1])    
    ...
    @property    
    def state(self):
        return [robot.state for color in self.robots for robot in self.robots[color]]
        ...
        for i, a in enumerate(action):
            ...
            if is_moved[i]:
                for blue_cell in self.board.blue_cells:
                    if blue_cell.robot:
                        if blue_cell.robot is not self.robots[a[0]][int(a[1])-1]:
                            blue_cell.robot.charge()  
        return any(is_moved)
    
    def reset(self, number_robots_with_same_color, robot_colors):
        ...
        robot_cells_init = random.sample(self.board.white_cells, k = self.number_robots)
        self.robots = {robot_color:[Robot(robot_cells_init[self.number_robots_with_same_color*j+i], i+1, robot_color, self.mail_sprites, self.game_clock) 
                       for i in range(self.number_robots_with_same_color)] for j,robot_color in enumerate(robot_colors) }
        ...
        while running:
            ...
            for i, color in enumerate(self.robots):
                for robot in self.robots[color]:
                    pygame.draw.circle(self.screen, self.color_map[robot.color], 
                                       (10.5*CELL_SIZE[0]+(robot.battery)*CELL_BATTERY_SIZE[0]+CELL_BATTERY_SIZE[0]/2,
